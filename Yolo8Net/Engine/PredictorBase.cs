...
namespace Yolov8Net
{
    abstract public class PredictorBase : IPredictor
    {
        ...
        protected PredictorBase(string modelPath, string[]? labels = null, bool useCuda = false, float confidence = 0)
        {
            ...
            if (Labels?.Length + 4 > ModelOutputDimensions)
                throw new ArgumentOutOfRangeException("Number of labels provided exceeds output dimensions of model.");
            if(confidence != 0)
        ...
        protected virtual DenseTensor<float>[] Inference(SixLabors.ImageSharp.Image img)
        {
            SixLabors.ImageSharp.Image resized;
        ...
        public virtual Prediction[] Predict(SixLabors.ImageSharp.Image image)
        {
