...
namespace Yolov8Net
{
        ...
        : PredictorBase, IPredictor
    {
        ...
        private List<Prediction> ParseDetect(DenseTensor<float> output, SixLabors.ImageSharp.Image image)
        {
        ...
        private List<Prediction> ParseSigmoid(DenseTensor<float>[] output, SixLabors.ImageSharp.Image image)
        {
            var result = new ConcurrentBag<Prediction>();
        ...
        protected List<Prediction> ParseOutput(DenseTensor<float>[] output, SixLabors.ImageSharp.Image image)
        {
            return UseDetect ? ParseDetect(output[0], image) : ParseSigmoid(output, image);

        }

        public override Prediction[] Predict(SixLabors.ImageSharp.Image image)
        {
            return Suppress(
             ParseOutput(
